...
for i in range(0, len(train_zero), gap):
    ...
    ngb = None
    ngb = NGBRegressor(Dist=Poisson, n_estimators=500, verbose=True, verbose_eval=5).fit(X_train, y_train)
    file_path = total_target + '_ngboost_zeros' + str(i) + '.p'
...
df.to_csv(path2 + 'all_External_poisson.csv', header=True, index=False)
print("finished")

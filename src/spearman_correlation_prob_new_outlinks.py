...
def plot_bar(avg_prediction, avg_prediction_stddev, regressor_prediction, regressor_prediction_stddev):
    ...
    fig.tight_layout()
    plt.savefig(fig_path + 'spearman_corr_orders_probOfNewOutlinks.png', dpi=500)
    plt.close()
    # plt.show()
...
for target in targets:
    ...
    for seed in seeds:
        ...
        # X_test_avg_outlinks = np.array([int_to_categorical(i) for i in np.rint(X_test['internalLinkChangeRate'].to_numpy())])
        # y_train = np.array([int_to_categorical(yi) for yi in y_train])
        # y_test = np.array([int_to_categorical(yi) for yi in y_test])
        X_test_link_changeRate = X_test['internalLinkChangeRate' if 'Internal' in target else 'externalLinkChangeRate'].to_numpy()
        y_train = np.array([int_to_categorical(yi) for yi in y_train])
        # y_test = np.array([int_to_categorical(yi) for yi in y_test])

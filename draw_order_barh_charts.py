...
dic_labels = {'link_change_rate': 'LCR', 'link_change_rate_ET': 'LCR-ET_{ALL}', 'related_link_change_rate': 'LCR-R',
              ...
              'LCRET-SPSNDP2DPRate': 'LCR-ET_{SP,SN,DN1,DP1}', 'LCRET-SPSN': 'LCR-ET_{SP,SN}',
              'random': 'random'}
...
"""dic_labels = {'link_change_rate': 'LCR', 'link_change_rate_ET': 'LCR-ET', 'related_link_change_rate': 'LCR-R',
              ...
              'LCRET-SPSNDP2DPRate': 'LCR-ET_{SP,SN,DP2,RLCR}', 'LCRET-SPSN': 'LCR-ET_{SP,SN}',
              'random': 'random'}
"""
...
def plot(dff, file_name, target):
    ...
    for col in cols:
        ...
        if 'ET' in col:
            ...
            lbl = 'random: area=0.5'
            z = -1
        ax.plot(x, dff[col] * 100, '--', color=colors[idx], linewidth=1, label=lbl, marker=markers[idx], markersize=4,
                zorder=z)
    ...
    plt.xlabel('% of hot pages selected from ' + 'each order')
...
for InEx in InExs:
    ...
    for target in targets:
        ...
        # fn_write = output_path + InEx + separator_in_file_name + dic_labels[
        #     target] + separator_in_file_name + 'round_' + str(
        #     round_decimal) + separator_in_file_name + 'gap_' + str(gap) + separator_in_file_name + (
        #                'nonZero' if remove_zero_values else 'all')
        fn_write = output_path + InEx + separator_in_filename + 'top_k_lineChart_order_' + dic_labels[
            target] + separator_in_filename + str(seed) + separator_in_filename + (
                       'nonZero' if remove_zero_values else 'all')
